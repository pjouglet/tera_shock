 ...
 */
public class ReplyRestoreHPMPStamina extends AbstractReply {

    private int price;

    public ReplyRestoreHPMPStamina(Node node){
        super(node);
    ...
    public void reply(Npc npc, Player player, Link link) {
        if(player.getCurrentHp() >= player.getMaxHp() && player.getCurrentMp() >= player.getMaxMp() && player.getStamina() >= player.getMaxStamina())
            return;
